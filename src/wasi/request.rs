...
impl Request {
    ...
    pub(crate) fn as_wstd(mut self) -> crate::Result<wstd::http::Request<wstd::http::Body>> {
        use wstd::http::{Body as WstdBody, Method as WstdMethod, Request as WstdRequest, Uri};
        let uri: Uri = self
            .url
            .to_string()
            .parse()
            .map_err(crate::error::builder)?;
        let mut req = WstdRequest::builder()
            .method(self.method)
            .uri(uri)
            .body(self.body.map(Body::as_wstd).unwrap_or_else(|| ().into()))
            .expect("request contents should be valid");
        std::mem::swap(req.headers_mut(), &mut self.headers);
        Ok(req)
    }
}
